...
console.log('开始更新 ShopPage.tsx 为本地图片路径...\n');
...
imagesData.forEach(item => {
  if (item.imageUrl) {
    const localPath = `/images/shop-${item.id}.jpg`;
    ...
    shopPageContent = shopPageContent.replace(regex, `$1${localPath}$3`);
    
    if (beforeUpdate !== shopPageContent) {
      console.log(`✓ 已更新: [${item.id}] ${item.name} -> ${localPath}`);
      updateCount++;
    } else {
