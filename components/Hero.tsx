    ...
    <section id={SectionId.HERO} className="relative min-h-screen flex items-center justify-center overflow-hidden bg-slate-50" style={{ zIndex: 'auto' }}>
      ...
      <div className="relative z-10 text-center px-4 max-w-4xl mx-auto select-none pt-20 pb-10 pointer-events-auto">
        <div className={`transition-all duration-1000 transform -mt-12 ${loaded ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
          ...
          <h1 className="text-5xl md:text-8xl font-bold text-slate-900 mb-6 leading-tight font-serif">
            {t('brand.name').split('：')[0]}：<span className="text-blue-900">{t('brand.name').split('：')[1]?.split('全球')[0]}</span><br/>
            <span className="text-4xl md:text-6xl">{t('brand.name').includes('全球') ? t('brand.name').split('全球')[1] : 'Global Activation'}</span>
          </h1>
          <p className="text-xl md:text-3xl text-blue-800 mb-4 max-w-3xl mx-auto leading-relaxed font-serif font-bold">
            {t('brand.tagline')}
          </p>
          <p className="text-base md:text-lg text-slate-600 mb-12 max-w-2xl mx-auto leading-relaxed">
            {t('brand.subtitle')}
          </p>
          ...
          <div className="flex flex-col md:flex-row gap-6 justify-center items-center">
            <button 
              ...
              className="bg-[#1e3a8a] text-slate-50 px-10 py-4 hover:bg-[#3b82f6] transition-colors duration-300 tracking-widest text-lg font-bold shadow-lg hover:shadow-blue-900/20"
            >
              {t('hero.cta1')}
            </button>
            <button 
              onClick={() => document.getElementById(SectionId.AIGC)?.scrollIntoView({behavior: 'smooth'})} 
              className="border-2 border-[#1e3a8a] text-[#1e3a8a] px-10 py-4 hover:bg-blue-50 transition-colors duration-300 tracking-widest text-lg font-bold"
            >
              {t('hero.cta2')}
            </button>
          </div>
