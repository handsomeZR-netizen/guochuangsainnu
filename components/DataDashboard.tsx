...
const TrendChart = memo(() => {
  ...
  return (
    <div ref={ref} className="bg-white p-6 border border-slate-100 shadow-xl shadow-slate-200/50 rounded-sm relative group hover:border-blue-200 transition-colors">
            ...
            <LazyResponsiveContainer width="100%" height="100%">
              <LazyAreaChart data={trendData}>
                ...
                <LazyYAxis 
                  ...
                  tickLine={false} 
                  axisLine={false}
                  label={{ value: '搜索热度', angle: -90, position: 'insideLeft', style: { fontSize: 10, fill: '#1e3a8a' } }}
                />
                <LazyYAxis 
                  yAxisId="right"
                  orientation="right"
                  ...
                  tickLine={false} 
                  axisLine={false}
                  label={{ value: '销售额(¥)', angle: 90, position: 'insideRight', style: { fontSize: 10, fill: '#3b82f6' } }}
                />
                <LazyTooltip 
                  contentStyle={{ 
                    backgroundColor: '#fff', 
      ...
      <div className="mt-3 flex items-start gap-2">
        <div className="bg-blue-50 p-1.5 rounded-full text-blue-800">
          <Activity size={14} />
        </div>
        <p className="text-xs text-slate-500 leading-relaxed">
          <span className="font-bold text-slate-800">增长趋势：</span> 
          南通蓝印花布搜索热度和销售额呈现稳定上升趋势，年度增长率达167%，市场需求持续旺盛。
        </p>
      </div>
    </div>
  );
...
const PriceComparisonChart = memo(() => {
  ...
  return (
    <div ref={ref} className="bg-white p-6 border border-slate-100 shadow-xl shadow-slate-200/50 rounded-sm hover:border-blue-200 transition-colors">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2 font-serif">
      ...
      <div className="h-44 w-full flex justify-center">
        {isInView ? (
          <Suspense fallback={<ChartPlaceholder />}>
            <LazyResponsiveContainer width="100%" height="100%">
              <LazyRadarChart cx="50%" cy="50%" outerRadius="75%" data={priceComparisonData}>
                <LazyPolarGrid stroke="#cbd5e1" strokeWidth={1} />
                <LazyPolarAngleAxis 
                  dataKey="platform" 
                  tick={{ fill: '#1e3a8a', fontSize: 12, fontFamily: 'serif', fontWeight: 600 }} 
                />
...
const GlobeSection = memo(() => {
  ...
  return (
    <div ref={ref} className="lg:col-span-7 h-[400px] lg:h-auto bg-slate-900 rounded-sm overflow-hidden relative shadow-2xl">
      {/* 3D Component */}
      ...
      <div className="absolute top-0 left-0 p-6 z-10 pointer-events-none w-full h-full flex flex-col justify-between">
        <div className="flex justify-between items-start">
          <div>
            <h3 className="text-2xl font-bold text-white flex items-center gap-2 font-serif mb-2">
              <Globe className="text-blue-400" size={24} /> 全球文化热力
            </h3>
            <p className="text-blue-200/70 max-w-sm text-xs">
              实时追踪非遗工艺品跨境流向。光点代表实时交易订单与物流轨迹。
            </p>
          </div>
          <div className="text-right hidden md:block">
            <div className="text-3xl font-bold text-white font-serif">4,821</div>
            <div className="text-blue-300 text-xs uppercase tracking-widest">今日活跃节点</div>
          </div>
        </div>

        {/* Stats overlay at bottom */}
        <div className="flex gap-6 border-t border-white/10 pt-4 backdrop-blur-sm bg-black/20 -mx-6 px-6 -mb-6 pb-4">
          <div>
            <div className="text-xl font-bold text-blue-400">42%</div>
            <div className="text-slate-400 text-xs uppercase">北美需求占比</div>
          </div>
          <div>
            <div className="text-xl font-bold text-white">28%</div>
            <div className="text-slate-400 text-xs uppercase">欧洲需求占比</div>
          </div>
          <div>
            <div className="text-xl font-bold text-white">18%</div>
            <div className="text-slate-400 text-xs uppercase">亚太需求占比</div>
          </div>
        </div>
      </div>
    </div>
...
const LiveTransactionFeed = memo(() => {
  ...
  return (
    <div className="w-full bg-slate-900/95 backdrop-blur-md text-blue-50 overflow-hidden py-3 mb-16 border-y border-blue-800 shadow-lg">
      <div className="container mx-auto px-6">
        ...
        <div className="space-y-1.5 max-h-32 overflow-hidden">
          {displayEvents.slice(0, 5).map((event, index) => (
            <div
              key={event.id}
              className="flex items-center gap-3 text-xs font-mono animate-slideIn"
              style={{ 
            ...
            >
              {getIcon(event.type)}
              <span className="text-slate-400">{getTimeAgo(event.timestamp)}</span>
              <span className="text-blue-200">来自{event.location}的用户</span>
              <span className="text-slate-400">{getActionText(event.type)}</span>
              <span className="text-white font-semibold">{event.product}</span>
              {event.price && (
    ...
    <section id={SectionId.DASHBOARD} className="py-24 bg-white/50 backdrop-blur-sm relative z-10">
      <div className="container mx-auto px-6">
        <div className="mb-16 text-center">
          ...
          <h2 className="text-4xl md:text-5xl text-slate-900 font-bold mb-4 font-serif">
            墨韵数屏 <span className="font-light italic text-blue-900">Intelligence</span>
          </h2>
          <div className="w-24 h-1 bg-gradient-to-r from-blue-900 to-blue-200 mx-auto mb-6"></div>
          <p className="text-slate-600 max-w-2xl mx-auto font-serif text-lg">
            实时价值评估 · 全球需求追踪 · 真伪溯源验证<br/>
            <span className="text-sm opacity-80 mt-2 block">由墨韵独家 AI 数据引擎驱动，让文化交易透明可视。</span>
          </p>
        </div>
